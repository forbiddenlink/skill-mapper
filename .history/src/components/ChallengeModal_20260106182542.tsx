...
import { useGameSounds } from '@/hooks/use-game-sounds'; // Using our new synth sounds
...
export default function ChallengeModal({
    ...
    const handleAnswer = (index: number) => {
        if (index === currentQ.correctIndex) {
            playClick(); // Positive blip
            if (currentQuestionIdx < quiz.length - 1) {
                setCurrentQuestionIdx(prev => prev + 1);
            } else {
                setStatus('success');
                playMastery(); // Victory sound
                setTimeout(() => {
                    onSuccess();
                    onClose();
        ...
        <AnimatePresence>
            <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
                <motion.div
                    ...
                    className="w-full max-w-lg bg-gray-900 border border-neon-cyan/50 rounded-lg p-6 shadow-[0_0_30px_rgba(0,243,255,0.15)] relative overflow-hidden"
                >
                    {/* Decorative Background Elements */}
                    <div className="absolute top-0 left-0 w-full h-1 bg-linear-to-r from-transparent via-neon-cyan to-transparent animate-pulse" />
                    ...
                    {/* Content */}
                    <div className="mb-8 min-h-50 flex flex-col justify-center">
                        {status === 'playing' && (
                            <motion.div
                                    ...
                                    {currentQ.options.map((opt, idx) => (
                                        ...
                                        >
                                            <span className="relative z-10 flex items-center gap-3">
                                                <span className="w-5 h-5 rounded-full border border-gray-600 flex items-center justify-center text-[10px] group-hover:border-neon-cyan group-hover:bg-neon-cyan group-hover:text-black">
