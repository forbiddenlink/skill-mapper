...
export function DailyChallenges() {
    ...
    const dailyChallenge = useMemo((): DailyChallenge | null => {
        ...
        const challengeSkill = availableSkills[seed % availableSkills.length] as typeof availableSkills[number];
        const quiz = challengeSkill.data.quiz as NonNullable<typeof challengeSkill.data.quiz>;
        const question = quiz[seed % quiz.length] as typeof quiz[number];
    ...
    const handleAnswerSubmit = () => {
        ...
        const seed = today.split('-').reduce((acc, val) => acc + parseInt(val), 0);
        const question = skill.data.quiz[seed % skill.data.quiz.length] as NonNullable<typeof skill.data.quiz>[number];
