...
class IndexedDBHelper {
    ...
    async saveState(state: any): Promise<void> {
        try {
            ...
            await new Promise<void>((resolve, reject) => {
                ...
                request.onerror = () => reject(new Error(request.error?.message || 'Failed to save state'));
            });
        } catch (error) {
    ...
    async loadState(): Promise<unknown | null> {
        try {
            ...
            const data = await new Promise<StoredGameState | undefined>((resolve, reject) => {
                ...
                request.onsuccess = () => resolve(request.result);
                request.onerror = () => reject(new Error(request.error?.message || 'Failed to load state'));
            });
    ...
    async clearState(): Promise<void> {
        try {
            ...
            await new Promise<void>((resolve, reject) => {
                ...
                request.onsuccess = () => resolve();
                request.onerror = () => reject(new Error(request.error?.message || 'Failed to clear state'));
            });
        } catch (error) {
    ...
    private fallbackLoad(): unknown | null {
        try {
