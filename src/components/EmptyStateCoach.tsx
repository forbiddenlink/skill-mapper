...
export default function EmptyStateCoach({ compact = false }: { compact?: boolean }) {
  const { nodes, streak, selectSkill, openFeaturesHub } = useGameStore(
    useShallow((s) => ({
      nodes: s.nodes,
      streak: s.streak,
      selectSkill: s.selectSkill,
      openFeaturesHub: s.openFeaturesHub,
    }))
  );
  ...
  const tips = [
    ...
    },
    {
      ...
      action: () => openFeaturesHub('streaks'),
    },
    {
      icon: Map,
      title: 'Follow a learning path',
      body: 'Open Learning Paths for a role-shaped roadmap.',
      action: () => openFeaturesHub('paths'),
    },
    {
      icon: Target,
      title: 'Daily challenge',
      body: 'One quiz a day keeps the streak honest.',
      action: () => openFeaturesHub('challenges'),
    },
  ];
